...
/**
 * @author semih on Eylül, 2020, 26.09.2020, 12:59:55
 * <p>github: <a href="https://github.com/SemihSz ">
 */
@Slf4j
@Service
@RequiredArgsConstructor
public class FilterDepthListService implements SimpleTask<EarthQuakeRequest, EarthQuakeResponse> {
	...
	public EarthQuakeResponse apply(EarthQuakeRequest request) {
		...
		try {
			...
			if (cacheList != null) {
				usageCountService.accept(Constant.Request.FILTER_DEPTH_LIST, ServiceUsageStatusType.SUCCESS);
				return EarthQuakeResponse.builder().quakeInfoList(cacheList.stream()
						.filter(t -> request.getDepth() < t.getDepth())
						.limit((Objects.nonNull(request.getLimit()) && request.getLimit() != 0) ? request.getLimit() : cacheList.size())
						.collect(Collectors.toList())).build();
			}
			else {
				usageCountService.accept(Constant.Request.FILTER_DEPTH_LIST, ServiceUsageStatusType.FAIL);
